...
//	  Условия к работе :
//	1. Объявления классов проводить в заготовочном файле(*.h);
//  2. Реализацию методов класса и функций делать в файле(*.cpp);
//	3. Главную функцию main() пишем в другом(в новом) файле(*.cpp);
//	4. Все поля класса должны быть частными.
//	5. Имена полей и методов класса должны соответствовать своей сути.
//	6. В деструкторе обязательно освобождать предварительно выделенную динамическую память.
//	7. При выполнении задания заданных в задаче необходимо использовать перегрузке операторы
//	8. Операторы условия(<, >, = =, != ) должны сравнивать среднеарифметическому значению всех элементов массива объекта.
